...
It seems that the file will calculate the velocity deficit for one turbine downwind of another turbine (only two turbines total with each turbine only having one position).
"""
...
# turbineY = np.zeros((relaxationFactor.size, 1))
turbineY = np.zeros((relaxationFactor.size, thetaVector.size))
# turbineYNormalized = np.zeros((relaxationFactor.size, 1))
...
# try hard-coding y values.
# turbineY = np.array([np.linspace(-2.0*rotorDiameter[0], 2.0*rotorDiameter[0], thetaVector.size),
#                      np.linspace(-2.0*rotorDiameter[0], 2.0*rotorDiameter[0], thetaVector.size),
#                      np.linspace(-2.0*rotorDiameter[0], 2.0*rotorDiameter[0], thetaVector.size),
#                      np.linspace(-2.0*rotorDiameter[0], 2.0*rotorDiameter[0], thetaVector.size),
#                      np.linspace(-2.0*rotorDiameter[0], 2.0*rotorDiameter[0], thetaVector.size)])
# turbineYNormalized = np.array([np.linspace(-2.0, 2.0, thetaVector.size),
#                                np.linspace(-2.0, 2.0, thetaVector.size),
#                                np.linspace(-2.0, 2.0, thetaVector.size),
#                                np.linspace(-2.0, 2.0, thetaVector.size),
#                                np.linspace(-2.0, 2.0, thetaVector.size)])

print turbineY.shape
print turbineYNormalized.shape
...
velocityFile = open('../DataFiles/JensenWECTestWindspeed.txt', 'w+')
...
velocityFile = open('../DataFiles/JensenWECTestWindspeed.txt', 'r')
...
v_over_u = np.zeros((relaxationFactor.size, thetaVector.size))
...
plt.title('WEC Jensen Model Velocity Deficit')
...
for i in range(relaxationFactor.size):
    ...
    plt.plot(turbineYNormalized[i, :], v_over_u[i, :], label=labelList[i])
    plt.ylabel(r'$V/U$')
...
plt.grid(True)
plt.legend()
plt.annotate(r'$x/r_0=%.1f$' % x_over_ro[0], xy=(-1.5, 1.0), xytext=(-2.0, 0.55), xycoords='data')
plt.show()
