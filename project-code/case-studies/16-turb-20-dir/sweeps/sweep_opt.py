...
from openmdao.api import Problem, pyOptSparseDriver, view_connections, SqliteRecorder, ExecComp
...
if __name__ == "__main__":
    ...
    if wec_method == 'diam':
        ...
        expansion_factors = np.exp(np.arange(0, 3, .2)[::-1])
    ...
    prob.model.add_objective('obj', scaler=1E-7)

    # select design variables
    prob.model.add_design_var('turbineY', lower=np.min(turbineY_init), upper=np.max(turbineY_init), indices=[turb])
    ...
    sweep_locs = np.loadtxt(input_directory + 'sweep_coords.txt')
    xsweep = sweep_locs[:, 0]
    ysweep = sweep_locs[:, 1]
    xaep = np.loadtxt(input_directory + 'sweep_xaep.txt')
    yaep = np.loadtxt(input_directory + 'sweep_yaep.txt')

    # x = np.zeros_like(expansion_factors)
    y = np.zeros_like(expansion_factors)
    # xaep_opt = np.zeros_like(x)
    yaep_opt = np.zeros_like(y)

    # xfig, xax = plt.subplots()
    yfig, yax = plt.subplots()
    # a45fig, a45ax = plt.subplots()

    for i in np.arange(0, 16, 4):
        yax.plot([turbineY[i], turbineY[i]], [0, -100], 'k--')

    xinit = turbineX_init[turb]+300.
    yinit = 0.75*np.max(turbineY_init)

    prob['turbineX'][turb] = xinit
    prob['turbineY'][turb] = np.copy(yinit)
    yopt = np.copy(yinit)
    # yopt = yinit
    ...
    if MODELS[model] is 'BPA':
        ...
        prob['model_params:calc_k_star'] = np.copy(calc_k_star_opt)
        if wec_method == 'diam':
            prob['model_params:wec_factor'] = np.copy(expansion_factors[0])
        elif wec_method == 'angle':
            prob['model_params:wec_spreading_angle'] = np.copy(expansion_factors[0])
    prob.run_model()
    yaep_init = np.copy(prob['AEP'])
    ...
    for expansion_factor, i in zip(expansion_factors, np.arange(0, expansion_factors.size)):  # best so far
        ...
        print('y before opt:', prob['turbineY'][turb])
        # quit()
        prob.run_driver()
        print('y after opt:', prob['turbineY'][turb])
        y[i] = np.copy(prob['turbineY'][turb])
        yaep_opt[i] = prob['AEP']
        ...
        yax.plot(y, -yaep_opt*1E-6, 'bo')

        # plot ysweep
        # yax.plot(ysweep, -yaep[i][:]*1E-6, label='%s %.3f' % (wec_method, expansion_factor))

        # plot ysweep
        # a45ax.plot(np.hypot(xsweep,ysweep), -a45aep[i][:]*1E-6, label='%s %.3f' % (wec_method, expansion_factor))
    yax.plot(yinit, -yaep_init * 1E-6, 'y^')
    sweep_exp_factors = np.exp(np.arange(0, 3, .2)[::-1])
    for i in np.arange(0, np.shape(yaep)[0]):
        yax.plot(ysweep, -yaep[i][:] * 1E-6, label='WEC-%s %.3f' % (wec_method, sweep_exp_factors[i]))
    print(y, yaep_opt)

    if wec_method == 'diam':
        prob['model_params:wec_factor'] = 1.
    elif wec_method == 'angle':
        prob['model_params:wec_spreading_angle'] = 0.
    prob['turbineY'][turb] = np.copy(yinit)
    prob.run_driver()
    yax.plot(prob['turbineY'][turb], -prob['AEP']*1E-6, '*c')
    yax.plot(yinit, -AEP_init_calc*1E-6, '^c')
    yax.legend()
    # yax.legend()
    # a45ax.legend()

    yax.set_xlabel('Turbine %i, Cross stream Location' % turb)
    # yax.set_xlabel('Turbine %i, Crosswind Location' % turb)
    # a45ax.set_xlabel('Turbine %i, distance from Origin' % turb)

    yax.set_ylabel('AEP (GWh)')
    # yax.set_ylabel('AEP (GWh)')
    # a45ax.set_ylabel('AEP (GWh)')

    yax.set_title('Y Sweep')
    # yax.set_title('Y Sweep')
    # a45ax.set_title('45 deg. Angle Sweep')
    print(ysweep[np.argmin(yaep[-1])])
    plt.show()