...
def run_opt(layout_number, wec_method_number, wake_model, opt_alg_number, max_wec, nsteps, InnerIter, OuterIter, record=True):
    ...
    if opt_algorithm == 'snopt':
        ...
        prob.model.add_constraint('boundaryDistances', lower=(np.zeros(nVertices * turbineX.size)), scaler=1E-7)
...
if __name__ == "__main__":
   ...
   # layout_number = int(sys.argv[1])
    ...
    for ii in np.arange(5, 31, 5):
        ...
        print("OuterIter=%i\n\n" %(oi))
        run_opt(layout_number, wec_method_number, model, opt_alg_number, max_wec, nsteps, ii.item(), oi, False)
