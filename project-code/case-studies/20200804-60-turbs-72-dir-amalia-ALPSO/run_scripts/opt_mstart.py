...
def run_opt(layout_number, wec_method_number, wake_model, opt_alg_number, max_wec, nsteps):
    ...
    layout_data = np.loadtxt(
        layout_directory + "layouts/amalia_60turbs/nTurbs60_spacing5_layout_%i.txt" % layout_number)
    ...
    if opt_algorithm == 'snopt':
        ...
        prob.driver.opt_settings["c2"] = 2.0  # # Social Parameter
    ...
    prob.model.add_design_var('turbineX', scaler=1E2, lower=np.zeros(nTurbines),
                              upper=np.ones(nTurbines) * 62. * rotor_diameter)
    prob.model.add_design_var('turbineY', scaler=1E2, lower=np.zeros(nTurbines),
    ...
    plt.plot(turbineX_init, turbineY_init, 'o')
    plt.plot(turbineX_end, turbineY_end, '*')
