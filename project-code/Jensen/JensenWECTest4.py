...
This file is built upon JensenWECTest2.py in that I'm trying to get the AEP vs. crosswind position for multiple
upwind turbines rather than just one upwind turbine.
"""
...
relaxationFactor = np.arange(7.0, 0.75, -1.0)
...
# For this run script, we only want three turbines - two upstream turbines that are generating wakes,
# and one downwind turbine receiving the wakes.
nTurbines = 3
# nTurbines = 2
...
# Enter the other turbines and their coordinates. Writing this code here because the separation distance depends on
# some of the other turbines' coordinates.
...
for i in range(relaxationFactor.size):
    ...
    turbineY[i] = np.array([np.linspace(-8.0*rotorDiameter[0], 8.0*rotorDiameter[0], thetaVector.size)])
...
AEPDataFile = open('../DataFiles/JensenWECTestAEPMultipleTurbines.txt', 'w+')
...
AEPDataFile = open('../DataFiles/JensenWECTestAEPMultipleTurbines.txt', 'r')
