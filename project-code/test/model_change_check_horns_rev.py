...
if __name__ == "__main__":
    ...
    ti_calculation_methods = np.array([0, 4, 5])  # can be [0:No added TI calculations,
    ...
    # These wake model options will overwrite other options later (e.g., Ct).
    ...
    normalized_power_NPA = np.zeros([6, nDirections])
    normalized_power_OURS = np.zeros([6, nDirections])
    ...
    #            np.c_[windDirections,
    #                  normalized_power_NPA[4, :], normalized_power_NPA[5, :],
    #                  normalized_power_OURS[4, :], normalized_power_OURS[5, :]],
    #            header="wind direction, NPA power, NPA power w/SM, our power, our power w/SM (%i rotor pts, %i model)" % (
